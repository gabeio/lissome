			...
			async.parallel [
				...
				->
					if req.headers['user-agent'].match(/BlackBerry|BB10|RIM Tablet/i) # Blackberry
						res.locals.mobile = true
				->
					if req.headers['user-agent'].match(/iP(hone|ad|od)/i) # Apple
						res.locals.mobile = true
				->
					if req.headers['user-agent'].match(/Opera Mini|Mini|mobile.+firefox/i) # usually "mini" things are mobile
						res.locals.mobile = true
				->
