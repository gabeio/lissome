	...
	describe "Blog", (...)->
		...
		after (done)->
			...
			err <- async.parallel [
				...
				(cont)->
					admin
						.get "/test/deleteposts/c/cps4601"
						.end (err, res)->
							cont err
			]
			done err
		...
		it "should not allow students not in the course to create new posts", (done)->
			err <- async.parallel [
				(cont)->
					student
						.post "/c/cps4601/blog?action=new"
						.send {
							"title":"title"
							"text":"student"
						}
				...
				(cont)->
					student
						.get "/c/cps4601/blog?action=new"
						.end (err, res)->
							expect res.status .to.not.equal 200
							cont err
				(cont)->
					student
						.get "/c/cps4601/blog?action=edit"
						.end (err, res)->
							expect res.status .to.not.equal 200
							cont err
				(cont)->
					student
						.get "/c/cps4601/blog?action=delete"
						.end (err, res)->
							expect res.status .to.not.equal 200
							cont err
			]
		...
		it "should not crash with garbage search date ranges", (done)->
			admin
				.get "/c/cps1234/blog?search=Jan+100+2014...Jan+200+2015"
				.end (err, res)->
					expect res.status .to.equal 200
					done err
		...
		it "should not allow student not in the course to edit or delete blog posts", (done)->
			err <- async.parallel [
				(cont)->
					student
						.post "/c/cps4601/blog/title?action=edit&hmo=PUT"
						.send {
							"pid":blogpid.1
							"title":"anything"
							"text":"anything"
				...
				(cont)->
					student
						.post "/c/cps4601/blog/title?action=delete&hmo=DELETE"
						.send {
							"pid":blogpid.1
						}
						.end (err, res)->
				...
				(cont)->
					student
						.post "/c/cps4601/blog/title?action=deleteall&hmo=DELETE"
						.send {
							"pid":blogpid.1
						}
						.end (err, res)->
		...
		it "should not allow faculty not in the the course to edit or delete blog posts", (done)->
			err <- async.parallel [
				(cont)->
					faculty
						.get "/c/cps4601/blog/title?action=edit"
						.end (err, res)->
							expect res.status .to.not.equal 200
							cont err
				(cont)->
					faculty
						.get "/c/cps4601/blog/title?action=delete"
						.end (err, res)->
							expect res.status .to.not.equal 200
							cont err
				(cont)->
					faculty
						...
						.end (err, res)->
							expect res.header.location .to.not.equal "/c/cps4601/blog/title?action=edit&success=yes"
				...
				(cont)->
					faculty
						...
						.end (err, res)->
							expect res.header.location .to.not.equal "/c/cps1234/blog?action=delete&success=yes"
							expect res.status .to.not.equal 302
							cont err
				(cont)->
					faculty
